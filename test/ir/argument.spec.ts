...
describe("Argument", () => {
    ...
    function createSinFn() {
        return llvm.Function.create(llvm.FunctionType.get(llvm.Type.getDoubleTy(context), [llvm.Type.getDoubleTy(context)], false), llvm.LinkageTypes.ExternalLinkage, "sin", module);
    }

    function createAddIntsFn() {
        const intTy = llvm.Type.getInt32Ty(context);
        return llvm.Function.create(llvm.FunctionType.get(intTy, [intTy, intTy], false), llvm.LinkageTypes.ExternalLinkage, "addInts", module);
    }
    ...
    describe("addDereferenceableAttr", () => {
       it("adds the dereferenceable attribute", () => {
           const fn = llvm.Function.create(llvm.FunctionType.get(llvm.Type.getVoidTy(context), [ llvm.Type.getInt32Ty(context).getPointerTo() ], false), llvm.LinkageTypes.ExternalWeakLinkage, undefined, module);
